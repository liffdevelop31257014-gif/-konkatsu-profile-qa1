...
function encodeDataToURL(data) {
  ...
  const compressed = btoa(encodeURIComponent(json)); // Base64エンコード
  const base        = location.href.split("?")[0].split("#")[0];
  return `${base}?share=${encodeURIComponent(compressed)}`;
}
...
function renderViewMode(data) {
  ...
  const rows = [
    ...
    { q: "Q8 ポジティブですか？ネガティブですか？",
       a: sliderVisual(data.q8 || 3, "ネガティブ", "ポジティブ") },
    { q: "Q9 周囲の感情などを察する方ですか？",
       a: sliderVisual(data.q9 || 3, "察さない", "察する") },
  ...
  const form      = document.querySelector(".container > *:not(.form-header)");

  // フォーム要素をすべて非表示
  document.querySelectorAll(
    ".container > label, .container > input, .container > textarea, .container > div:not(.form-header)"
  ).forEach(el => el.style.display = "none");

  // ビューエリアを表示
  container.style.display = "block";

  const itemsHTML = rows.map(({ q, a }) => `
    <div class="view-item">
      <p class="view-question">${q}</p>
      <p class="view-answer">${a.replace(/\n/g, "<br>")}</p>
    </div>
  `).join("");
  ...
  container.innerHTML = `
    <div class="view-header">
      ...
      ${data._shareName ? `<p class="view-name">${data._shareName} さんの回答</p>` : ""}
    </div>
    ${itemsHTML}
  `;
}
...
(async () => {
  ...
  document.querySelectorAll('input[name="q4"]').forEach((radio) => {
    radio.addEventListener("change", () => toggleDetail("q4Detail", radio.value === "yes"));
  });

  /* ----- Q7 ラジオ：詳細テキストエリアの表示制御 ----- */
  document.querySelectorAll('input[name="q7"]').forEach((radio) => {
    radio.addEventListener("change", () => toggleDetail("q7Detail", radio.value === "yes"));
  });
  ...
  document.getElementById("shareBtn").addEventListener("click", async () => {
    ...
    const messageText = shareName
      ? `${shareName} の自己開示QA回答です。\n${shareURL}`
      : `自己開示QA Part1 の回答です。\n${shareURL}`;
