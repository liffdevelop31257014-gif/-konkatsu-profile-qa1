...
function sliderVisual(value, leftLabel, rightLabel, max = 5) {
  ...
  const bar = "◉".repeat(v) + "〇".repeat(max - v);
  ...
  container.innerHTML = `
    ...
    ${rows.map(({ q, a }) => `
      <div class="view-item">
        ...
        <p class="view-answer">${escapeHTML(a).replace(/\n/g, "<br>")}</p>
      </div>
    `).join("")}
  `;
}
    ...
    if (liff.isApiAvailable("shareTargetPicker")) {
      try {
        const result = await liff.shareTargetPicker(
          ...
          { isMultiple: true }   // 複数の友だちに同時送信可
        );
        if (result) {
          alert("送信しました！");
        }
        // result が undefined のときはキャンセル（何もしない）
      } catch (e) {
        console.error("shareTargetPicker error:", e);
        // shareTargetPicker が使えない場合のフォールバック
        promptCopyURL(shareURL);
      }
    } else {
      // LIFF外（PCブラウザ等）ではURLコピーで代替
      promptCopyURL(shareURL);
    }
  });
...
/* ------------------------------------------------------------
   フォールバック：URLコピー（LINEアプリ外のとき）
   ------------------------------------------------------------ */
async function promptCopyURL(url) {
  try {
