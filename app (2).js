...
function renderViewMode(data) {
  ...
  container.innerHTML = `
    ...
    <div class="cta-card">
      <img src="image2.png" class="cta-flower" alt="">
      <img src="image1.png" class="cta-clipboard" alt="">
...
async function sendShareMessageToSelf(previewMsg) {
  const available = liff.isInClient() && liff.isApiAvailable("sendMessages");
  alert(`[debug] isInClient: ${liff.isInClient()} / sendMessages available: ${liff.isApiAvailable("sendMessages")}`);
  try {
    if (available) {
    ...
    alert("[debug] sendMessages error: " + e);
